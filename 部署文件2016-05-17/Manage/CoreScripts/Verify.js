...
Verify.prototype.Init=function()
{
	...
	tbox.after(star);
	var tbox=$("textarea[nullable='false']");
	tbox.after(star);
...
Verify.prototype.SubmitEvent=function(){
	$("input[type='submit'][verify='true']").click(btnEvent);
	$("input[type='image'][verify='true']").click(btnEvent);
	function btnEvent(){			
			//按钮的验证组，识标码
...
Verify.OperateSingle=function(group,control){
	...
	{		
		...
		if(group==null || group=="" || group==tbgroup)
		{
			...
			//if(!Verify.FileSize(control))return false;	
		}
	}
	return isPass;
}
...
Verify.ShowBox=function(control,showTxt){
	...
	box.html(showTxt);
	box.css("width",control.width()<200 ? 200 : control.width());
	box.css("height",30);	
	box.css("top",offset.top+control.height()+5);
	box.css("left",offset.left);
	box.css("background-color","#D1FFA4");
	var alpha=90;
	box.css("filter","Alpha(Opacity="+alpha+")");
	box.css("display","block");
	box.css("-moz-opacity",alpha/100);
	box.css("opacity",alpha/100);
	box.css("border","1px solid #66CC00");
	box.slideDown("slow");
}
//获取当前输入控件的录入值
...
Verify.IsNull=function(control){
	...
	if(nullable==null || nullable=="")return isPass;
	//取录入的值
...
Verify.Lenth=function(control){
    ...
    if (typeof (limit) == "undefined") return isPass;
    //取录入的值长度
    ...
    if (limit.indexOf("-") < 0) {
        maxlen = Number(limit);
        if (len > maxlen) {
            Verify.ShowBox(control, " 请限制 " + limit + " 个字符");
            isPass = false;
            control.focus();
        }
    } else {
        ...
        if (!(len >= minlen && len <= maxlen)) {
            Verify.ShowBox(control, " 请录入 " + minlen + "-" + maxlen + " 个字符");
            isPass = false;
            control.focus();
        }
    }
...
Verify.NumberLimit=function(control)
{
	...
	{		
		Verify.ShowBox(control," 请录入>="+minlen+"且<="+maxlen+" 的数字。");
		isPass=false;
		control.focus();
	}
	return isPass;
...
Verify.IsSame=function(control)
{
	...
	if(sametarget!=null)
	{
		var same=$("*[id$='"+sametarget+"']");
		if(val!=Verify.GetValue(same))
		{			
			Verify.ShowBox(same," 输入不相同！");
			isPass=false;
			same.focus();
		}
	}
...
Verify.DataType=function(control)
{
	...
	if(datatype=="user"){		
		...
		if (!patrn.exec(val)){
			Verify.ShowBox(control," 请用字符开头的字串！");
			isPass=false;
			control.focus();
		} 
	}
