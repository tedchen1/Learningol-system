...
$(function(){		
		...
		//获取新消息个数
		setInterval("getMessage()",10*1000);	
		//判断链接
...
//获取短消息个数
function getMessage(){
	$().SoapAjax("Message","NewNumber",{accId:emplyeeId},messSecc,null,null);
	function messSecc(data)
	{
		var num=Number($(data).text());	
		var img=$("#messageMarker img");
		img.attr("src","Images/ICO/message"+(num==0 ? 1 : 2)+".gif");
	}
}
//隐藏遮罩层
