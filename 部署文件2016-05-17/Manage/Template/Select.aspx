...
<div class="typeTitle">电脑Web模板</div>
    <div class="ListBox">
            ...
            <asp:Repeater ID="rtpTemplate" runat="server">
                <ItemTemplate>
                    <div class="<%# Convert.ToBoolean(Eval("isCurrent")) ? "item itemCurrent" : "item"%>"
                        couid="<%# Eval("path") %>" tag="<%# Eval("tag") %>" type="Web" wd="600" hg="400">
                        ...
                        <div class="select">
                            <asp:LinkButton ID="btnSelect" OnClick="btnSelectWeb_Click" CommandArgument='<%# Eval("tag") %>'
                                Visible='<%# !Convert.ToBoolean(Eval("isCurrent")) %>' runat="server">采用该模板</asp:LinkButton>
    ...
    <br />
    <hr />
    <div class="typeTitle">手机Wap模板</div>
    <div class="ListBox">
        <div class="ListBoxInner">
        <asp:Repeater ID="rtpTempMobi" runat="server">
                <ItemTemplate>
                    <div class="<%# Convert.ToBoolean(Eval("isCurrent")) ? "item itemCurrent" : "item"%>"
                        couid="<%# Eval("path") %>" tag="<%# Eval("tag") %>" type="Mobile" wd="600" hg="400">
                        <img src="<%# Eval("logo")+"?s="+DateTime.Now.Ticks %>" />
                        <div class="itemName">
                            <%# Eval("name") %><%# Eval("size","（{0}）") %>
                        </div>
