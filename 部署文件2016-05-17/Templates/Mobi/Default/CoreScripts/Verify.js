...
Verify.prototype.SubmitEvent = function () {
    var btn = $("*[type='submit'][verify='true']");
    btn.click(
		function () {
		    //按钮的验证组，识标码
...
Verify.ShowBox = function (control, showTxt) {
    ...
    box.css({ "position": "absolute", "z-index": 2000,
        "height": 25, "width": control.width() < 60 ? 100 : control.width() + 3,
    ...
    box.find(".errorShow").css({ "border": "1px solid ##FF3300",
        ...
        "text-indent": "10px", "height": "25px", "width": control.width() < 60 ? 100 : control.width() + 3,
...
Verify.IsNull = function (control) {
    ...
    return nullable == "false" && val == "" ? Verify.ShowBox(control, "不得为空！") : true;
}
//验证录入的长度区间
...
Verify.IsSame = function (control) {
    ...
    var val = Verify.GetValue(control);
    if (sametarget != null) {
        var same = $("*[name$='" + sametarget + "']:visible");
