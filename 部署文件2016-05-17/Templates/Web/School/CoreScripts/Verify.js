...
Verify.prototype.Init = function () {
    ...
    //tbox.after(star);
    var tbox = $("textarea[nullable='false'][start!='false']");
    tbox.after(star);
...
function setError() {
    var acc = $().getPara("acc");
    var idcard = $().getPara("idcard");
    if (acc != "") {
        var txt = $("input[name=tbAcc]:visible");
        txt.val(acc);
        $("input[name=tbIDCard]:visible").val(idcard);
    }
    //错误信息处理
