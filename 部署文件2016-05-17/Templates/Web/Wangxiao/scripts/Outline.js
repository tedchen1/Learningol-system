...
function timeHandler(t) {
    //播放进度
    if (t > -1) {
        CKobject._K_('playTime').innerHTML = t;
        var id = $().getPara('id');
...
//加载视频播放器
var flashvars = {
    f: window.BASE64.decoder(videoPath), //视频地址
...
function closelights() {//关灯
    box.Show();
    CKobject._K_('videobox').style.width = '720px';
    CKobject._K_('videobox').style.height = '550px';
    CKobject.getObjectById('ckplayer_videobox').width = 720;
    CKobject.getObjectById('ckplayer_videobox').height = 550;
}
